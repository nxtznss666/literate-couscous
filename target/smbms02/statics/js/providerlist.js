...
function deleteProvider(obj){
	$.post(path+"/provider/delProvider","id="+obj.attr("proid"),function(data){
		alert(123)
		if(data.result=="true"){
			cancleBtn();
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/provider/getProvider.html?id="+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/provider/toModifyProvider.html?id="+ obj.attr("proid");
	});
